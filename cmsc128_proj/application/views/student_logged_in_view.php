...
<html>
	<head>
		...
		<link rel = "stylesheet" type = "text/css" href= "<?php echo base_url();?>assets/css/home_student.css">
		<title>Adviser</title>
	</head>
	...
	<body>
		...
		<p id="studentNum"><?=$username?>, <?=$role?></p>
	</div>
		...
		<div id="content">
			...
			<div class="panel-heading">
				<h4>NOTIFICATIONS</h4>
				<form method="POST" action="<?php echo site_url('student_control/studentShowNotifs'); ?>">
			      <button type="submit" id="notifs">Show Notifications</button>
			    </form>
			</div>
                      ...
                      <?php $row_count = count($result['msgs']);
                            if($row_count != 0) {
                              foreach ($result['msgs'] as $row) {
                                ...
                                echo form_open('Student_controller/rejectRequest');
                                echo form_hidden('msg_id', $row->notif_id);
